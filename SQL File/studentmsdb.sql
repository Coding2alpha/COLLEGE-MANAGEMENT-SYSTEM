...
CREATE TABLE tblfaculty (
  ...
  FacultyID varchar(200) DEFAULT NULL,
...
CREATE TABLE `tblnotice` (
  ...
  CREATE TABLE hod (
  department varchar(3) NOT NULL UNIQUE,
  HOD varchar(50) default "NA",
  primary key(department)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
INSERT INTO `hod` (department) VALUES
('CSE'),
('ECE'),
('BS');
select FacultyName from tblfaculty where department ="CSE";
...
SELECT tblclass.ID,tblclass.Faculty,tblclass.ClassName from tblclass where tblclass.Faculty="new";


SELECT info.marks, info.attendance
from info 
JOIN tblstudent ON tblstudent.StuID = info.studentID
JOIN tblclass ON tblclass.ClassName = info.courseName 
WHERE info.courseName="ece" && info.studentID=17;
